...
public class SynthesisManager : MonoBehaviour
{
    ...
    private void PerformSynthesis(SynthesisRecipe recipe, List<IconData> usedIcons)
    {
        ...
        // 2. 清空歷史紀錄中的這些圖示
        var history = IconManager.Instance.GetSynthesisHistory();
        foreach (var icon in usedIcons)
        {
            if (history.Contains(icon))
            {
                // 需要用反射或公開方法來移除
                // 暫時先用 RemoveFromSynthesis 已經處理了
            }
        }
        
        // 3. 在結果槽顯示新圖示
        if (resultSlot != null && recipe.resultIcon != null)
        {
            resultSlot.Setup(recipe.resultIcon);
            Debug.Log($"[SynthesisManager] 結果槽顯示：{recipe.resultIcon.displayName}");
        }
        
        // 4. 將新圖示解鎖到顯示區
        if (recipe.resultIcon != null)
        {
            bool added = IconManager.Instance.AddIcon(recipe.resultIcon);
            if (added)
            {
                Debug.Log($"[SynthesisManager] 新圖示已解鎖：{recipe.resultIcon.displayName}");
            }
        }
        ...
        // 6. 可選：顯示成功訊息
        // ShowSuccessMessage($"獲得：{recipe.resultIcon.displayName}");
    }
